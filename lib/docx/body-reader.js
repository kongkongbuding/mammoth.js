...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            return {
                ...
                // fontSize: element.firstOrEmpty('w:sz').attributes['w:val'] || element.firstOrEmpty('w:szCs').attributes['w:val']
            };
        });
    }
    ...
    var xmlElementReaders = {
        ...
        "w:pPr": function(element) {
            ...
            return readParagraphStyle(element).map(function(style) {
                var alignment = element.firstOrEmpty("w:jc").attributes["w:val"]
                return {
                    type: "paragraphProperties",
                    styleId: style.styleId,
                    styleName: style.name || alignment,
                    alignment: alignment,
    ...
    function readBlip(element, blip) {
        ...
        var extent = element.first("wp:extent").attributes;
        var altText = isBlank(properties.descr) ? properties.title : properties.descr;
        return readImage(findBlipImageFile(blip), altText, extent);
    }
    ...
    function readImage(imageFile, altText, extent) {
    ...
    function readSectPrElement(element) {
        var pgSz = element.firstOrEmpty("w:pgSz").attributes;
        var pgMar = element.firstOrEmpty("w:pgMar").attributes;
        // 单位：in（英寸）
        var emuWeight = 1440;
        var w = pgSz["w:w"] / emuWeight;
        var h = pgSz["w:h"] / emuWeight;
        var footer = pgMar["w:footer"] / emuWeight;
        var gutter = pgMar["w:gutter"] / emuWeight;
        var header = pgMar["w:header"] / emuWeight;
        var left = pgMar["w:left"] / emuWeight;
        var right = pgMar["w:right"] / emuWeight;
        var top = pgMar["w:top"] / emuWeight;
        var bottom = pgMar["w:bottom"] / emuWeight;
